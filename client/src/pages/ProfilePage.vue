<template>
  <div class="space-y-4">
    <h2 class="text-lg font-semibold text-slate-800 md:text-xl">My Profile</h2>
    <section class="grid gap-3 rounded-xl border border-slate-200 bg-white p-3 shadow-sm sm:p-4 md:max-w-2xl">
      ...
      <form v-else class="grid gap-3" @submit.prevent="handleSave">
        ...
        <div class="flex flex-col gap-2 sm:flex-row">
          <button
            :disabled="saving"
            type="submit"
            class="w-full rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-60 sm:w-fit"
          >
            {{ saving ? 'Saving...' : 'Save Profile' }}
          </button>
          <button
            type="button"
            class="w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-100 sm:w-fit"
            @click="cancelEdit"
          >
            Cancel
          </button>
    ...
    <section class="grid gap-3 rounded-xl border border-slate-200 bg-white p-3 shadow-sm sm:p-4 md:max-w-2xl">
      <h3 class="text-base font-semibold text-slate-800 md:text-lg">Change Password</h3>
      <PasswordInput
